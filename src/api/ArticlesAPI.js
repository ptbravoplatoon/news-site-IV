...
const searchArticles = async (text) => {
	const filterText = `?filter={"where":{"title":{"ilike":"${text}"}}}`;
	const response = await fetch(`${BASE_URL}${filterText}`);
	const data = await response.json();
	return data;
};

export { fetchArticleByID, fetchArticles, fetchArticlesBySection, searchArticles };
